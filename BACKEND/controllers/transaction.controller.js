...
exports.updateTransaction = async (req, res) => {
  try {
    const { id } = req.params; // could be order_id or collect_id
    ...
    // Try to find by collect_id first, then by custom_order_id
    const transaction = await OrderStatus.findOneAndUpdate(
      { $or: [{ collect_id: id }, { custom_order_id: id }] },
      updateData,
      { new: true }
    );
